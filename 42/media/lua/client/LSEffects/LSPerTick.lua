...
local dbDelay = false
local dbCount = 0
local LSInvSyncTick = 0       -- [NR_Patch] throttle counter for CLSInv.UpdateInvScripts
local LSInvSyncInterval = 120 -- [NR_Patch] run every ~2s at 60fps instead of every tick
	...
	if thisPlayer ~= nil and playerData.LSMoodles ~= nil then
		...
		if not thisPlayer:isAsleep() then--when player is asleep some functions won't play
			...
			if SandboxVars.Text.DividerArt then
				-- [NR_Patch] Fix: UpdateInvScripts iterated all inventory containers every tick,
				-- causing 275ms+ spikes. Throttled to every LSInvSyncInterval ticks (~2s at 60fps).
				LSInvSyncTick = LSInvSyncTick + 1
				if LSInvSyncTick >= LSInvSyncInterval then
					LSInvSyncTick = 0
