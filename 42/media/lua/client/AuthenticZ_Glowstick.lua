-- AuthenticZ_Glowstick.lua
-- Patched by MrTheSteve_Patches.
-- [NR_Patch] Fix: glowstickUpdate ran the full inventory walk and tore down/rebuilt
-- all light sources every frame via OnPlayerUpdate, regardless of whether the player
-- had any glowsticks equipped. Added early-out: if no active glowsticks are found
-- and no lights are currently registered for this player, return immediately.
...
local function glowstickUpdate(player)
	...
	-- [NR_Patch] Early-out: no active glowsticks and no lights to clean up — nothing to do.
	local hasLights = lightByPlayer[player] ~= nil and next(lightByPlayer[player]) ~= nil
	if #workingInvGlowsticks == 0 and not hasLights then return end

	-- Only set needsRefresh when there is actual glowstick work to do.
